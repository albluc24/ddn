...
class L2SamplerWithAlphaChannelTopk:
    def __init__(self, target, topk=2):
    ...
    def __call__(self, dic):
        ...
        probs = nn.functional.softmax(
            -((rgbs - target) ** 2 * alpha).sum([-1, -2]).mean(-1)
            / (alpha.sum() + 1e-6),
            0,
        )
...
if __name__ == "__main__":
    ...
    condition_rgb = imread("/home/yl/dataset/ffhq/test_self/test_self/FFHQ-test4.png")
