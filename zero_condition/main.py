...
class L2Sampler():
    ...
    def __call__(self, dic):
        ...
        resized = nn.functional.interpolate(
                       self.target, (h, w), mode="area"
                    )
        probs = nn.functional.softmax(-((rgbs-resized)**2).mean([-1,-2, -3]), 0)
        return dict(probs=probs, idx_k=int(probs.argmax()), condition=self.target, condition_source=self.raw)
...
class L1Sampler(L2Sampler):
    ...
    def __call__(self, dic):
        ...
        probs = nn.functional.softmax(-(torch.abs(rgbs-resized)).mean([-1,-2, -3]), 0)
        return dict(probs=probs, idx_k=int(probs.argmax()), condition=self.target, condition_source=self.raw)
...
class NoiseSampler(L2Sampler):
    def __init__(self, target, noise_rate=.53):
        self.raw = target
...
class SuperResSampler():
    def __init__(self, target, shape=.5):
        self.raw = target
...
class CannySampler():
    ...
    def __call__(self, dic):
        ...
        k, h, w, c = rgbs.shape
        hh, ww = self.target.shape[-2:]
        rgbs = nn.functional.interpolate(
                rgbs, (hh, ww), mode="nearest"
            )
        rgbs = rgbs.permute(0, 2, 3, 1).cpu().numpy()    # Change tensor shape to (k, h, w, c)
        ...
        resized_imgs = np.stack(resized_imgs, axis=0)    # Stacking the images into one numpy array
        # if w == 64:
        #     g()/0
        ...
        probs=resized_imgs[:,target_resized>0].mean(-1) + (1 - resized_imgs[:,target_resized<.5].mean(-1))
        # probs = - probs
        if h <=16:
...
class EdgeSampler():
    ...
    def __call__(self, dic):
        ...
        # probs=  probs[:,0]
        probs = -(torch.abs(resized_imgs - target_resized).clip(0.)).float().mean(-1).mean(-1).mean(-1)
        # probs = - probs
        idx_k = int(probs.argmax())
        g()
        if not increase("edge show") % 8:
            show(resized_imgs[:1], rgbs[:1], t2rgb)
        return dict(probs=probs, idx_k=idx_k, condition=self.target, condition_source=resized_imgs[0])
...
class StyleTransfer():
    def __init__(self, target):
        ...
        pretrain = torchvision.models.resnet18(weights=torchvision.models.resnet.ResNet18_Weights.DEFAULT).state_dict()
...
class CifarSampler():
    ...
    def __call__(self, dic):
        ...
        return dict(probs=probs, idx_k=idx_k, condition=target,)
    
    
class BatchedGuidedSampler():
    def __init__(self, sampler=None):
        sampler = sampler or (lambda d:dict(probs=[1.]+[0]*(len(d["rgbs"])-1)))
...
classi2name={0: 'airplane', 1: 'automobile', 2: 'bird', 3: 'cat', 4: 'deer', 5: 'dog', 6: 'frog', 
...
if __name__ == "__main__":
    ...
    # for classi in range(10):
        ...
        print(classi, classi2name.get(classi, "None"), "class:",sampler.model(d["predict"]).max(1)[1].item())
